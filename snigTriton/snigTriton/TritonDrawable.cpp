...
static const size_t NUM_CONTEXTS = 64;
    ...
    */
    ...
    {
        PassHeightMapToTritonCallback(ByyTritonContext* triton) : _TRITON(triton), _enable(false), _id(0) { _ptrEnable = const_cast<bool*> (&_enable); };
        virtual void operator()( osg::RenderInfo& renderInfo ) const
        {
            if( _enable )
            {
                *_ptrEnable = false; // cannot directly change _enable because of const
				...
				//printf("PassHeightMapToTritonCallback\n");
				renderInfo.getCurrentCamera()->setNodeMask(0x0);
            }
        }
    ...
    {
    public:
        OceanTerrainChangedCallback(ByyTritonDrawable* drawable)
            : _drawable(drawable) { }
    ...
    public: // osgEarth::TerrainCallback
        ...
        {
            osg::ref_ptr<ByyTritonDrawable> drawable;
            if ( _drawable.lock(drawable) )
                drawable->dirtyAllContexts();
        }

    private:
        osg::observer_ptr<ByyTritonDrawable> _drawable;
    };
...
ByyTritonDrawable::ByyTritonDrawable(osgEarth::MapNode* mapNode, ByyTritonContext* TRITON) :
_TRITON(TRITON),
_mapNode(mapNode)
,myLastHeightMapUpdateTime(0)
{
...
ByyTritonDrawable::~ByyTritonDrawable()
{
...
void
ByyTritonDrawable::dirtyAllContexts()
{
    _contextDirty.setAllElementsTo(1);
}

void
ByyTritonDrawable::updateHeightMap(osg::RenderInfo& renderInfo) const
{
    if ( !_TRITON->ready() )
        return;
...
void
ByyTritonDrawable::drawImplementation(osg::RenderInfo& renderInfo) const
{
    ...
    if ( !_TRITON->ready() )
        return;
	//printf("drawImplementation\n");
    if ( _TRITON->passHeightMapToTriton() && !_terrainChangedCallback.valid() )
    {
        const_cast<ByyTritonDrawable*>(this)->setupHeightMap(*state);
    }
    ...
    if ( _TRITON->passHeightMapToTriton() )
    {
		...
		double curTime=osg::Timer::instance()->time_s();

		dirty=dirty&&curTime-myLastHeightMapUpdateTime>=2;

        if ( dirty )
        {
			_heightCamera->setNodeMask(0x1);
			...
			myLastHeightMapUpdateTime=curTime;
        }
    }
...
void ByyTritonDrawable::setupHeightMap(osg::State& state)
{
    osg::ref_ptr<osgEarth::MapNode> mapNode;
    if (!_mapNode.lock(mapNode))
        return;
