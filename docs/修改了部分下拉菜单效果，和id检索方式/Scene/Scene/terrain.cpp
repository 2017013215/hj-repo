...
bool ByyTerrain::crushNode(const osgEarth::GeoPoint& point, int& opdId, osg::Vec3d& orientation, osgEarth::GeoPoint& crushObjPt)
{
	...
	for (;it!=nodesList.end();++it)
	{
		...
		//qDebug() << __FUNCTION__ << __LINE__ << qAbs(tPoint.x() - point.x()) << qAbs(tPoint.y() - point.y()) << "BBBBBBBBBBBBBBBBBBBBBBBBBBBBB";
		//在一定范围内，自动选择
		if((qAbs(tPoint.x() - point.x()) < 0.001) && (qAbs(tPoint.y() - point.y()) < 0.003))
		{
			crushObjPt = tPoint;
			int tmpOpdId = it.value();
			ByyEntityObject* obj = ByyIG::IGInstance()->core().localObjectManager().getObjectByGlobalId(tmpOpdId);
...
{
	bool ret = false;
	ByyEntityObject* obj = ByyIG::IGInstance()->core().localObjectManager().getObjectByGlobalId(opdId);
	if(obj)
	{
