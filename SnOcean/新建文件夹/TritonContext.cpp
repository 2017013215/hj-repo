    ...
    if ( !_initAttempted && !_initFailed )
    {
        ...
        if ( !_initAttempted && !_initFailed )
        {
            ...
            if ( err == ::Triton::SUCCEEDED )
            {
				...
				_environment->SetSeaLevel(2);

				Triton::BreakingWavesParameters wavesParam=_environment->GetBreakingWavesParameters();
				wavesParam.SetWavelength(100);
				wavesParam.SetWavelengthVariance(100);
				//_environment->SetBreakingWavesParameters(wavesParam);
