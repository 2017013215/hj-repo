...
namespace ArsBase
{
    ...
    //-----------------------------------------------------------------------
    Vec3 Matrix3::GetColumn (size_t iCol) const
    {
    ...
    //-----------------------------------------------------------------------
    void Matrix3::SetColumn(size_t iCol, const Vec3& vec)
    {
        assert( 0 <= iCol && iCol < 3 );
        m[0][iCol] = vec.x;
        m[1][iCol] = vec.y;
    ...
    //-----------------------------------------------------------------------
    Vec3 Matrix3::operator* (const Vec3& rkPoint) const
    {
        Vec3 kProd;
        for (size_t iRow = 0; iRow < 3; iRow++)
        {
    ...
    //-----------------------------------------------------------------------
    Vec3 operator* (const Vec3& rkPoint, const Matrix3& rkMatrix)
    {
        Vec3 kProd;
        for (size_t iRow = 0; iRow < 3; iRow++)
        {
    ...
    //-----------------------------------------------------------------------
    void Matrix3::SingularValueDecomposition (Matrix3& kL, Vec3& kS,
        Matrix3& kR) const
    {
    ...
    void Matrix3::SingularValueComposition (const Matrix3& kL,
        const Vec3& kS, const Matrix3& kR)
    {
        size_t iRow, iCol;
        Matrix3 kTmp;
    ...
    //-----------------------------------------------------------------------
    void Matrix3::ToAxisAngle (Vec3& rkAxis, Radian& rfRadians) const
    {
        ...
        rfRadians = Math::ACos(fCos);  // in [0, ONE_PI ]

        if ( rfRadians > Radian(0.0) )
        {
            if ( rfRadians < Radian(Math::ONE_PI) )
            {
    ...
    //-----------------------------------------------------------------------
    void Matrix3::FromAxisAngle (const Vec3& rkAxis, const Radian& fRadians)
    {
        ...
        Vec3 akEigenvector[3]) const
    {
        ...
        Vec3 kCross = akEigenvector[1].crossProduct(akEigenvector[2]);
    ...
    //-----------------------------------------------------------------------
    void Matrix3::TensorProduct (const Vec3& rkU, const Vec3& rkV,
        Matrix3& rkProduct)
    {
        for (size_t iRow = 0; iRow < 3; iRow++)
        {
