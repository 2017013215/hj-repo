...
namespace ArsBase {
	...
	//-----------------------------------------------------------------------
    void Quaternion::ToAngleAxis (Radian& rfAngle, Vec3& rkAxis) const
    {
        // The quaternion representing the rotation is
        //   q = cos(A/2)+sin(A/2)*(x*i+y*j+z*k)
	...
	//-----------------------------------------------------------------------
    Vec3 Quaternion::xAxis(void) const
    {
        //Real fTx  = 2.0*x;
        Real fTy  = 2.0*y;
        Real fTz  = 2.0*z;
	...
	//-----------------------------------------------------------------------
    Vec3 Quaternion::yAxis(void) const
    {
        Real fTx  = 2.0*x;
        Real fTy  = 2.0*y;
        Real fTz  = 2.0*z;
	...
	//-----------------------------------------------------------------------
    Vec3 Quaternion::zAxis(void) const
    {
        Real fTx  = 2.0*x;
        Real fTy  = 2.0*y;
        Real fTz  = 2.0*z;
	...
	bool Quaternion::equals(const Quaternion& rhs, const Radian& tolerance) const
	{
		...
		return (Math::Abs(angle.valueRadians()) <= tolerance.valueRadians())
            || Math::RealEqual(angle.valueRadians(), Math::ONE_PI, tolerance.valueRadians());
	}
        ...
        const Quaternion& rkP, const Quaternion& rkQ, int iExtraSpins)
    {
        ...
        Real fSin = Math::Sin(fAngle);
        Radian fPhase ( Math::ONE_PI*iExtraSpins*fT );
