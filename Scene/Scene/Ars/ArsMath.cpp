...
namespace ArsBase
{
	...
	const Real Math::ONE_PI = Real( 4.0 * atan( 1.0 ) );
	const Real Math::TWO_PI = Real( 2.0 * ONE_PI );
	const Real Math::HALF_PI = Real( 0.5 * ONE_PI );
	const Real Math::fDeg2Rad = ONE_PI / Real(180.0);
	const Real Math::fRad2Deg = Real(180.0) / ONE_PI;
	...
	void Math::buildTrigTables(void)
	{
		...
		// Could get away with building only ONE_PI sized Sin table but simpler this 
	...
	{
		return Real( ArsASM::asm_rsq(fValue));
	}

	//-----------------------------------------------------------------------
	Real Math::UnitRandom ()
	{
		return ArsASM::asm_rand() / ArsASM::asm_rand_max();
	}
	...
	//-----------------------------------------------------------------------
	bool Math::pointInTri2D(const Vec2& p, const Vec2& a, 
	...
	//-----------------------------------------------------------------------
	bool Math::pointInTri3D(const Vec3& p, const Vec3& a, 
	...
	//        const Vec3& rayorig = ray.getOrigin() - sphere.getCenter();
	...
	//    }
	//    //-----------------------------------------------------------------------
	//	std::pair<bool, Real> Math::intersects(const Ray& ray, const AABox& box)
	//	{
	...
	//    }
	//    //-----------------------------------------------------------------------
	//    std::pair<bool, Real> Math::intersects(const Ray& ray, const Vec3& a,
	//        const Vec3& b, const Vec3& c, const Vec3& normal,
	...
	//        bool positiveSide, bool negativeSide)
	//    {
	//        Vec3 normal = calculateBasicFaceNormalWithoutNormalize(a, b, c);
	...
	//    }
	//    //-----------------------------------------------------------------------
	//    Vec3 Math::calculateTangentSpaceVector(
	...
	//	    Vec3 normal = side1.crossProduct(side0);
	//	    normal.normalise();
	...
	//	    Vec3 tangentCross = tangent.crossProduct(binormal);
	...
	//    }
	//    //-----------------------------------------------------------------------
	//    Vec4 Math::calculateFaceNormal(const Vec3& v1, const Vec3& v2, const Vec3& v3)
	//    {
	//        Vec3 normal = calculateBasicFaceNormal(v1, v2, v3);
	//        // Now set up the w (distance of tri from origin
	//        return Vec4(normal.x, normal.y, normal.z, -(normal.dotProduct(v1)));
	//    }
	//    //-----------------------------------------------------------------------
	//    Vec3 Math::calculateBasicFaceNormal(const Vec3& v1, const Vec3& v2, const Vec3& v3)
	//    {
	...
	//    }
	//    //-----------------------------------------------------------------------
	//    Vec4 Math::calculateFaceNormalWithoutNormalize(const Vec3& v1, const Vec3& v2, const Vec3& v3)
	//    {
	//        Vec3 normal = calculateBasicFaceNormalWithoutNormalize(v1, v2, v3);
	//        // Now set up the w (distance of tri from origin)
	//        return Vec4(normal.x, normal.y, normal.z, -(normal.dotProduct(v1)));
	//    }
	//    //-----------------------------------------------------------------------
	//    Vec3 Math::calculateBasicFaceNormalWithoutNormalize(const Vec3& v1, const Vec3& v2, const Vec3& v3)
	//    {
	//        Vec3 normal = (v2 - v1).crossProduct(v3 - v1);
	//        return normal;
	//    }
		...
		Real Math::gaussianDistribution( Real x, Real offset, Real scale)
		{
			...
			Real denom = scale * Math::Sqrt( 2 * Math::ONE_PI );
	
			return nom / denom;
		}

	//	//---------------------------------------------------------------------
	//	Matrix4 Math::makeViewMatrix(const Vec3& position, const Quaternion& orientation, 
